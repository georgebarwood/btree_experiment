...
impl<K, V> Tree<K, V> {
    ...
    fn get_pos_mut(&mut self, pos: &[usize], level: usize) -> &mut (K, V) {
        match self {
    ...
    fn get_pos(&self, pos: &[usize], level: usize) -> &(K, V) {
        match self {
    ...
    fn remove_pos(&mut self, pos: &[usize], level: usize) -> (K, V) {
        match self {
...
impl<K, V> Leaf<K, V> {
    ...
    fn prepare_insert(&mut self, pos: &mut PosVec) -> Option<(Tree<K, V>, (K, V))> {
        if self.full() {
            ...
            if pos[level] >= LEAF_SPLIT {
                pos[level] -= LEAF_SPLIT;
                pos[level - 1] += 1;
            }
            Some(self.split())
        } else {
...
impl<K, V> NonLeaf<K, V> {
        ...
        K: Ord,
    {
        ...
        if self.full() {
            ...
            if pos[level] >= NON_LEAF_SPLIT {
                pos[level] -= NON_LEAF_SPLIT;
                pos[level - 1] += 1;
            }
            Some(self.split())
        } else {
